...
const main = async () => {
  ...
  for (let i = movesBeginIndex; i < lines.length; i++) {
    ...
    let elements: Stack = new Stack();
    for (let j = 0; j < numberToMove; j++) {
      elements.push(stacks[fromStack].pop()!);
    }
    // pop the elements off the elements stack and push them onto the toStack
    while (!elements.isEmpty()) {
      stacks[toStack].push(elements.pop()!);
    }
  }
