...
class Monkey {
  ...
  constructor(monkeyId: number, items: number[], operationOperator: string, operationNumber: number, testDivisibilityNum: number, trueMonkeyNum: number, falseMonkeyNum: number) {
  ...
  public conductMonkeyBusiness(monkeys: Monkey[]): Monkey[] {
    ...
    for (const item of newMonkeyItems) {
      const tempOperationNumber: number = this.operationNumber === -1 ? item : this.operationNumber;
      const newWorryLevel = Math.floor((operators[this.operationOperator](item, tempOperationNumber)) / 3);
...
function getTwoHighestInspectedMonkeys(monkeys: Monkey[]): number[] {
  ...
  for (let monkey of monkeys) {
    ...
    } else if (monkey.timesInspected > secondHighestInspectedMonkey.timesInspected) {
...
function parseMonkey(lines: string[]): Monkey {
  ...
  for(let i = 0; i < lines.length; i++) {
    ...
    if (i === 0) {
      ...
      startingItemsWorryLevels = line.trim().split(" ")[2].split(",").map((item) => parseInt(item));
    } else if (i === 2) {
